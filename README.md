...
> 服务器分享、WebDAV 备份在 v1.1.0 加入；自检预检在 v1.1.10 加入；启动失败智能提示、端口占用一键处理、未保存改动保护在 v1.1.11 加入。
> v1.2.0 加入：系统代理接管提速（只接管活动网络服务 + VPN 冲突自动跳过）、运行中切换服务器、检查更新「隧道优先、失败降级直连」。
> v1.2.1 加入：服务器分享导入/导出的成功与失败弹窗反馈；失败弹窗统一短文案（完整原因保留在运行日志）；结果弹窗统一原生 NSAlert 并置于屏幕上部居中。
> v1.2.2 修复：自动更新后新实例不自动重启（脚本直接执行二进制启动）、旧进程未退净导致双菜单栏图标、替换成功后自动删除下载的 DMG、退出时记录代理状态并在下次启动自动恢复。
> v1.2.3 优化：启动新实例前无条件清理旧实例并预留系统清图标时间，消除更新期间双菜单栏图标短暂并存。
> v1.2.4 修复：崩溃/强杀后接管标记残留导致下次启动**图标假蓝**（端口无人监听），初始不再信任残留标记，真正接管成功才变蓝；启动时检测到残留接管自动还原系统代理自愈。
> v1.2.5 更新：开机自启动关闭后系统「允许在后台运行」开关残留为开启（launchd「已启用」记录未翻转，现用 `launchctl disable` 清理），下拉箭头颜色恢复系统默认；新增敏感字段掩码（服务器地址 / TOKEN）、WebDAV 删除备份 / 删除服务器、核心配置字段重排与默认优选域名 `cdns.doon.eu.org`，并修复 Swift 6 并发告警、更换应用图标。
> v1.2.6 修复：敏感字段掩码不显示（掩码层画在输入框下层被背景盖住），移到输入框之上并用系统语义色，深浅色模式均清晰；App 图标适配 Mac 风格。
> v1.2.7 更新：窗口高度改为隐藏探针按内容理想高度驱动（含用户手动拉伸保护）；运行日志展开时恰好显示 4 行无残影、修复日志底部被窗口截断；状态栏云朵图标放大（18pt→22pt）；服务器下拉框宽度只随内容、不再被拉宽。
...
| 版本 | 内容 |
|---|---|
| 1.2.9 | **隧道性能大幅优化**：系统根证书池全局缓存 + TLS 会话复（多连接建连提速）；移除预热池与并发握手闸门（简易协议直连，不再被 6 并发限死）；移除黑洞检测（拖动 3 秒续播、长时稳定）；HTTP CONNECT 建好隧道才回 200（修复 1 秒短连卡顿）+ bufio 首包保护；转发/WS 缓冲 256KB；严格遵循分流模式（不再自动改走直连）。实测 YouTube 峰值 9W~15W、大流量持续 7.8MB/s |
| 1.2.8 | **自检逻辑重构**：手动自检判定服务器地址（根路径 `WebSocket Proxy Server`）+ Token 鉴权（真实升级握手，与服务端不一致直接报出）；未配置 Token 也照常校验，不再跳过；手动自检结果统一弹窗；自检通过后才接管系统代理、失败则彻底关闭代理。**DoH 兼容**：显式启用 HTTP/2，支持只走 HTTP/2 的 DoH（如 `doh.onedns.net`）。**协议探测**：超时 4s→10s，探测失败按 simple 处理（服务端为简易 WebSocket 代理时不再误判 smux 导致黑洞） |
